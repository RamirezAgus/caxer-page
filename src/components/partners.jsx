...
export default function Parterns() {
    return(
        <section className='bg-white w-full min-h-screen py-10'>
            <h2 className='text-center text-2xl py-10 md:py-20 sm:text-3xl md:text-4xl lg:text-5xl font-bold text-cyan-500'>NUESTROS PARTNERS</h2>
            <div className="flex justify-center items-center">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-18 md:gap-x-14 lg:gap-40">
                    {logos.map((logo, index) => (
                        <img key={index} src={logo.src} alt={logo.alt} className="w-32 auto scale-up"/>
                    ))}
                </div>
            </div>
        </section>
